...
import debounce from './hooks/throttlingHandler';
...
function App() {
    ...
    // prevent repeated calling before 1000 ms
    const respond = debounce((e)=>{handlesearch(e)},1000);
                ...
                <div className="grid grid-cols-12 sm:px-5 gap-x-8 gap-y-16">
                    {
                        filtereddata.map((elem,index)=>{
                                return (
                                    <Card elem={elem} index = {index} key={index}/>
                                )
                            })
                    }
                </div>
